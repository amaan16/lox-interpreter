...
class Parser {
  ...
  private Stmt classDeclaration() {
    ...
    while (!check(RIGHT_BRACE) && !isAtEnd()) {
      ...
      methods.add(method(isStatic ? "static method" : "method", isStatic));
    }
  ...
  private Stmt.Function method(String kind, boolean isStatic) {
    Token name = consume(IDENTIFIER, "Expect " + kind + " name.");
    if (match(LEFT_PAREN)) {
      List<Token> parameters = new ArrayList<>();
      if (!check(RIGHT_PAREN)) {
      ...
      error(peek(), "Expect '(' for method or '{' for getter.");
      return null; 
    }
  }
